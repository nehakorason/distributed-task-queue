...
class TestJobHandlerRegistry(unittest.TestCase):
    ...
    def test_all_expected_handlers_registered(self):
        expected = {
            "data_processing", "email_notification",
            "report_generation", "file_sync", "database_migration",
        }
...
class TestWorkerApiCall(unittest.TestCase):
    ...
    def test_api_call_returns_none_on_any_exception(self):
        w = self._make_worker()
        # Patch requests.request to raise a plain Exception
        # and patch requests.RequestException to be Exception
        # so the except clause catches it correctly
        with patch("worker.requests.request", side_effect=Exception("network error")), \
             patch("worker.requests.RequestException", Exception):
...
class TestWorkerJobProcessing(unittest.TestCase):
    ...
    def test_process_job_success_increments_processed(self):
        ...
        w.running = True
        job_data = {
            "id": "job-1", "type": "file_sync",
            "payload": '{"files":["a.txt"]}',
            "retryCount": 0, "maxRetries": 3,
        }
        with patch.object(w, "_api_call") as mock_api:
            mock_api.side_effect = [{}, job_data, {}]
    ...
    def test_process_job_always_removes_from_processing_set(self):
        ...
        def api_side_effect(*args, **kwargs):
            ...
            if call_count[0] == 1:
                ...
                raise Exception("kaboom")   # fetch fails
            return {}          # fail report OK

        with patch.object(w, "_api_call", side_effect=api_side_effect):
            w.redis_client.srem = MagicMock()
    ...
    def test_process_job_failure_increments_failed(self):
        ...
        def api_side_effect(*args, **kwargs):
            ...
            if call_count[0] == 1:
                ...
                raise Exception("fetch failed")  # fetch fails
            return {}          # fail report OK
